  ...
  if (!me) {
    return (
      <MobileShell showTabs activeTab="affiliation">
        <div className="flex h-full items-center justify-center text-zinc-400">{error ?? '로딩...'}</div>
      </MobileShell>
    );
  }
  ...
  return (
    <MobileShell showTabs activeTab="affiliation">
      <div className="px-7 pt-10 pb-8">
        ...
        <p className="mt-1 text-sm text-zinc-500">팀장에 따라 자동으로 정해져요. 변경은 관리자에게 문의하세요.</p>
      </div>
...
function ReadField({ label, value }: { label: string; value?: string | null }) {
  return (
    <div>
      ...
      <div className="mt-2 w-full rounded-[5px] bg-zinc-50 px-5 py-4 text-lg font-bold text-zinc-900 ring-1 ring-zinc-200">
