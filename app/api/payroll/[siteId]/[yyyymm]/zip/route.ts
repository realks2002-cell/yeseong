...
export async function GET(
  ...
  // subcontractor_name별 그룹핑 (NULL → '미배정')
  ...
  for (const [subName, groupSlots] of groups) {
    ...
    const innerName = safeFileName(`노임대장_${subName}_${data.worksite.name}_${yyyymm}.xlsx`);
    zip.file(innerName, buf);
  }

  const zipBuf = await zip.generateAsync({ type: 'nodebuffer' });
  const zipFilename = safeFileName(`노임대장_${data.worksite.name}_${yyyymm}.zip`);
