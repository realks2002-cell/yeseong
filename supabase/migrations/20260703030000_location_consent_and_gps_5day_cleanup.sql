-- 위치정보(GPS) 수집·이용 동의를 가입 동의서에 추가 + GPS 로그 보관 30일→5일
--   1) yeseong_workers 에 consent_location_at 컬럼 추가
--   2) yeseong_mobile_signup_full v7: p_consent_location 파라미터 추가 (필수 동의)
--   3) yeseong_manager_signup_full: p_consent_location 파라미터 추가 (필수 동의)
--   4) yeseong_cleanup_gps_logs: 보관 기간 30일 → 5일
...
comment on column yeseong_workers.consent_location_at is
  '위치정보(GPS) 수집·이용 동의 시각 (위치정보보호법 / PIPA §15, 필수)';

-- ============================================================
-- 2) yeseong_mobile_signup_full v7 — p_consent_location 추가
-- ============================================================
...
create or replace function yeseong_mobile_signup_full(
  ...
  p_consent_location boolean default false
) returns uuid
...
begin
  ...
  if coalesce(p_consent_location, false) = false then
    raise exception 'consent_location_required';
  end if;
  ...
  insert into yeseong_workers (
    ...
    team_leader_id,
    auth_user_id,
    consent_personal_at, consent_rrn_at, consent_location_at,
...
grant execute on function yeseong_mobile_signup_full(
  text, text, text, text, text, text, text, text, integer, text,
  boolean, text, text, text, text,
  boolean, boolean, boolean, boolean, text, uuid, boolean
) to authenticated;

-- ============================================================
-- 3) yeseong_manager_signup_full — p_consent_location 추가
-- ============================================================
...
-- ============================================================
-- 4) GPS 로그 보관 기간 30일 → 5일
--    사전 고지(위치 권한 화면)에서 "5일 후 자동 삭제"로 안내하는 것과 일치시킨다.
-- ============================================================
create or replace function yeseong_cleanup_gps_logs()
returns void
language sql
as $$
  delete from yeseong_gps_logs where created_at < now() - interval '5 days';
$$;
