-- 팀장 가입: phone 매칭 실패 시 name fallback
--   관리자가 phone 없이 미리 등록한 site_manager(예: 김장열 phone=null)와
--   본인 가입자(phone 입력)를 같은 row로 연결하기 위함.
--
--   매칭 우선순위:
--     1) auth_user_id = 현재 사용자 (재가입)
--     2) site_managers.phone = v_phone (관리자가 phone 미리 채운 경우)
--     3) site_managers.name = v_resolved_name AND phone is null (관리자가 이름만 등록)
--     4) 매칭 없으면 INSERT
