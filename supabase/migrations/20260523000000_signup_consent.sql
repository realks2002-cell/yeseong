-- 모바일 작업자앱 — 개인정보 수집 동의 (PIPA §15, §24, §24조의2, §17)
--   1) yeseong_workers에 동의 컬럼 추가
--   2) yeseong_mobile_signup_full v5: 동의 파라미터 추가, 필수 동의 누락 시 차단

-- ============================================================
-- 1) yeseong_workers 컬럼 추가
--    timestamptz 사용 — null이면 미동의, 값이 있으면 그 시점에 동의함
-- ============================================================
alter table yeseong_workers
  add column if not exists consent_personal_at     timestamptz,
  add column if not exists consent_rrn_at          timestamptz,
  add column if not exists consent_foreign_id_at   timestamptz,
  add column if not exists consent_third_party_at  timestamptz,
  add column if not exists consent_version         text;

comment on column yeseong_workers.consent_personal_at    is '개인정보 수집·이용 동의 시각 (PIPA §15, 필수)';
comment on column yeseong_workers.consent_rrn_at         is '주민등록번호 수집 동의 시각 (PIPA §24조의2, 별도 동의 필수)';
comment on column yeseong_workers.consent_foreign_id_at  is '외국인등록번호 수집 동의 시각 (PIPA §24, 외국인일 때 필수)';
comment on column yeseong_workers.consent_third_party_at is '제3자 제공 동의 시각 (PIPA §17, 선택)';
comment on column yeseong_workers.consent_version        is '동의 약관 버전 (예: 1.0)';


-- ============================================================
-- 2) yeseong_mobile_signup_full v5 — 동의 파라미터 추가
--    시그니처 변경 → drop 후 재정의
-- ============================================================
